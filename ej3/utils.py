...
def binning(bins: Iterable[Union[int, float]], val: float):
    i = 0
...
def arr_binning(bins: Iterable[Union[int, float]], x: np.array):
    return np.array(list(map(lambda v: binning(bins, v), x)))
